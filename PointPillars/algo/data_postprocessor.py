...
class data_postprocessor(object):
    ...
    def data_postprocess(self, model_outputs, filenames):
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            name_2_id_map = {'Pedestrian': 1, 'Mbike': 2, 'Car': 3, 'Bus': 4, 'Tricycle': 5}
            pred_labels = [name_2_id_map[i] for i in pred_labels]
            ...
            #             'position': pred_boxes[i, :3].tolist(),
            #             'dimension': pred_boxes[i, 3:6].tolist(),
            #             'rotation': [0, 0, pred_boxes[i, 6]],
            #             'velocity': [0, 0, 0],
            #             'acceleration': [0, 0, 0],
            #             'description': {}
            #         })
            ...
            if num_preds != 0:
                for i in range(num_preds):
                    ...
                    anno_temp.update({
                        ...
                        'cluster_id': i,
                        'dimension': pred_boxes[i, 3:6].tolist(),
                        'location_image': [[-1000, -1000], [-1000, -1000], [-1000, -1000]],
                        "occluded": -1,
                        'position': pred_boxes[i, :3].tolist(),
                        'rotation': [0, 0, float(pred_boxes[i, 6])],
                        "score": 1.0,
                        "track_id": 1010,
                        "truncated": -1,
                        "type": pred_labels[i],
