...
LastEditTime: 2023-02-02 16:53:26
'''
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class Predictor(object):
    def __init__(self, model, data_loader, data_root, predictions_root, params_dict, res_dict):
        self.model = model
    ...
    def Predict(self):
        ...
        with torch.no_grad():
            ...
            for inputs, labels, filenames, idx in self.data_loader:  ## for each batch
                ...
                predictions, filenames, task_type = self.data_postprocessor.data_postprocess(outputs, filenames)

                for prediction, filename in zip(predictions, filenames):
                    print('predicting ' + filename)
                    self.res_dict['msg'].append('predicting ' + filename)
                    ...
                    #         if label['task'] == task_type:
                    #             label['annotation']['annotation'] = prediction['annotation']
                    # else:
                    #     label_list = [{'annotation': {'annotation': prediction['annotation'], 'frameDescription': {}, 'tags': [], 'quality': {}}, 'task_type': task_type}]
                    #
                    # if not os.path.exists(self.predictions_root):
                    #     os.makedirs(self.predictions_root)
                    ...
                    label = {'data_path': '', 'file_name': name, 'sync_time': 0.0}
                    object3d = {'lidar_main': {'annotation': prediction['annotation'], 'seq_id': int(name), 'time_stamp': 0.0}}
                    label['object3D'] = object3d
                    ...
                    src_pcd_file = os.path.join(self.data_root, 'Lidar', filename)
                    dst_pcd_file = os.path.join(self.predictions_root, name[-6:] + '_lidar_main.pcd')
                    import shutil
                    shutil.copy(src_pcd_file, dst_pcd_file)
