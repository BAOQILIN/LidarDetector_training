...
class LidarDetector(object):
    def __init__(self, params_dict, data_root, result_root, model_epoch_root='', load_model=True, load_data=True, train_flag=True, test_flag=True, res_dict={}):
        ...
        if load_model:
            self.model_component = model_components(self.params_dict.copy(), self.result_root, self.model_epoch_root, res_dict=self.res_dict)
        if load_data:
    ...
    def predict(self, predictions_root, epoch_id):
        ...
        predictor = Predictor(self.get_model(epoch_id), self.get_dataloader(),
                              self.data_root, predictions_root, self.params_dict, self.res_dict)
        predictor.Predict()         
